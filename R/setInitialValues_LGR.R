#' @title DABOM Initial Values - LGR
#'
...
setInitialValues_LGR = function(filter_ch,
                                parent_child,
                                configuration) {
  ...
  branch_df = getNodeInfo_LGR(parent_child,
                              configuration) %>%
  ...
  # build node order
  no = parent_child %>%
    PITcleanr::addParentChildNodes_LGR(configuration = configuration) %>%
