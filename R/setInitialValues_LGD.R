...
setInitialValues_LGD = function(dabom_list = NULL) {
  ...
  a_init = array(0, dim=c(n.fish, n_branch_list$n.pops.main))
  ...
  a_init[, 1:(n_branch_list$n.pops.main - 1)] = sapply(dabom_list,
                                                 function(x) apply(x, 1, max, na.rm = T))
  # initial black box
  a_init[,n_branch_list$n.pops.main] = abs(apply(a_init, 1, max, na.rm=T) - 1) #not seen anywhere
  # a_init[,n_branch_list$n.pops.main + 1] = NA #not seen anywhere
