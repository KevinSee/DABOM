#' @title Modify basic JAGS model - LGR
#'
...
#' @export
#' @return NULL
#' @examples fixNoFishNodes_LGR()

fixNoFishNodes_LGR = function(init_file = NULL,
                              file_name = NULL,
                              filter_ch = NULL,
                              parent_child = NULL,
                              configuration = NULL,
  ...
  node_info = getNodeInfo_LGR(parent_child, configuration) %>%
    full_join(filter_ch %>%
                group_by(node) %>%
