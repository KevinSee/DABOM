...
# which configuration nodes are not in the parentchild table?
config %>%
  filter(!Node %in% union(unique(parentchild$ParentNode), unique(parentchild$ChildNode))) %>%
  filter(lubridate::year(EndDate) >= 2015) %>%
  xtabs(~ Node, .)
...
#------------------------------------------------------------------------------
fish_obs <- fishPaths(valid_obs_dat,
                      valid_paths)
...
fish_obs2 <- spawnerPaths(valid_obs_dat,
                          valid_paths)
