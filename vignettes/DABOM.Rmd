...
`DABOM` has two companion R packages, [PITcleanr](https://github.com/BiomarkABS/PITcleanr) and [STADEM](https://github.com/BiomarkABS/STADEM). `PITcleanr` can assist the user in "cleaning" sometimes unwieldy amounts of detection data from [PTAGIS](https://ptagis.org/) and preparing that detecion data for `DABOM`. `STADEM` can be used to estimate adult escapement at a dam that, when combined with movement probabilities from `DABOM`, provide an estimate of abundance or escapement at locations of interest.


# Set-up

<<<<<<< HEAD:vignettes/DABOM.Rmd
...
The first step is to ensure that all appropriate software and R packages are installed on your computer. [R](https://cran.r-project.org/) is a language and environment for statistical computing and graphics and is the workhorse for running all of the code and models described here. R packages are collections of functions and data sets developed by the R community for particular tasks. Some R packages used here are available from the general R community and available [here](https://cran.r-project.org/web/packages/available_packages_by_name.html)) whereas others (`PITcleanr`, `DABOM`) are developed by the package author and available on [GitHub here](https://github.com/BiomarkABS).
>>>>>>> update_2.0:vignettes/Run-DABOM.Rmd

`DABOM` requires the [JAGS](http://mcmc-jags.sourceforge.net/) software for Bayesian inference, which can be downloaded from [here](https://sourceforge.net/projects/mcmc-jags/files/).
